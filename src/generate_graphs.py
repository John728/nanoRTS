...
def generate_SNR_data_vs_SNR_test(
    model_func,
    snr_training_range: np.arange,
    snr_test_range: np.arange,
    path: str,
    num_avg: int = 10,
    verbose: int = 1,
):
    ...
    for data_snr in snr_training_range:
        ...
        for test_snr in snr_test_range:
            ...
            for i in range(num_avg):
                ...
                if verbose:
                    sys.stdout.write(
                        f"\rCalculating loss for SNR: {test_snr} ({i}/{num_avg})"
                    )
                ...
                noise = generate_gaussian_noise_SNR(
                    num_samples=1000, SNR=test_snr, signal=rts
                )
...
def generate_data_size_vs_time(
    model_function, data_size_range: np.arange, file_name: str, verbose: int = 1
):
    """
    ...
    for data_size in data_size_range:
        ...
        time_to_run = model_function(
            x_train, y_train, x_valid, y_valid, (num_samples, 1), "./autoencoder_model"
        )
...
def generate_loss_vs_noise(
    model, snr_range: np.arange, file_name: str, num_avg: int = 10, verbose: int = 1
):
    """
    ...
    if verbose:
        print(
            f"Generating loss vs. SNR graph, each point averaged over {num_avg} runs..."
        )
    ...
    for target_snr in snr_range:
        ...
        for i in range(num_avg):
            if verbose:
                sys.stdout.write(
                    f"\rGenerating loss for SNR: {round(target_snr, 2)}, iteration {i + 1}"
                )
                sys.stdout.flush()
    ...
    loss_vs_snr = {"loss": loss, "snr": snr_range}
...
def generate_loss_vs_time(
    model_func, data, path: str, num_avg: int = 10, verbose: int = 1
):
    ...
    _, hist = model_func(
        X_train, Y_train, X_valid, Y_valid, (1000, 1), verbose=1, save_model=False
    )
...
def generate_SNR_animation(model, SNR_range, path, starting_count=0):
    ...
    for target_SNR in SNR_range:
        ...
        sys.stdout.write(
            f"\rGenerating SNR animation, iteration {i + 1}/{len(SNR_range)}"
        )
        ...
        # Generate the noise based on the desired SNR
        noise = generate_gaussian_noise(
            num_samples=1000, mean=0, std=np.sqrt(noise_avg_watts), seed=0
        )
        ...
        plt.plot(
            ...
            color="purple",
            alpha=0.2,
            label="Noisy RTS Signal",
        )
        plt.plot(rts, color="red", linewidth=1, label="RTS Signal")
        plt.plot(
            predicted_rts, color="orange", linewidth=2, label="Predicted RTS Signal"
        )
        plt.xlabel("Time")
